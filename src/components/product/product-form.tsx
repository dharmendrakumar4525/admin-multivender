...
import {
	AttachmentInput, Category, Product, ProductStatus, ProductType, Tag, Type, VariationOption
...
const competitorPricing = (price: any, min_price: any) => {
	const amount = price || min_price;
	return [
		{ 'name': 'faire', 'price': Number((amount * 0.35) + amount) },
		{ 'name': 'bzaar', 'price': Number((amount * 0.45) + amount) },
		{ 'name': 'qalara', 'price': Number((amount * 0.5) + amount) },
	]
}
			...
			updateProduct(
				{
					variables: {
						...
						input: {
							...inputValues,
							competitors_pricing: competitorPricing(inputValues?.price, inputValues?.min_price),
						},
					},
				},
				{
